...
import graphviz
import lingam
from lingam.var_lingam_mod import VARLiNGAM
...
# model.adjacency_matrices_ returns an array [B0_est, B1_est, ...]
...
# Optional: Print true vs estimated matrices for comparison
# print("\nTrue B0:\n", B0_true)
# if 'B0_est' in locals(): print("Estimated B0:\n", B0_est)
# print("\nTrue B1:\n", B1_true)
# if 'B1_est' in locals(): print("Estimated B1:\n", B1_est)



# --- Assume you have these variables defined ---
# model = ... # Your fitted VarLiNGAM model instance
# B0_true = np.array(...) # Your true B0 matrix
# B1_true = np.array(...) # Your true B1 matrix

# --- IMPORTANT: Obtain the PRUNED matrices ---
# Make sure estimated_matrices holds the matrices AFTER pruning
# Example: If pruning was done separately:
# B0_est_pruned = your_pruning_function(model.adjacency_matrices_[0], ...)
# B1_est_pruned = your_pruning_function(model.adjacency_matrices_[1], ...)
# estimated_matrices = [B0_est_pruned, B1_est_pruned]
# Or if the model's .adjacency_matrices_ already holds pruned results:
...
# Ensure B0_true and B1_true are NumPy arrays
B0_true = np.array(B0)
B1_true = np.array(B1)
...
# Optional: Print true vs estimated pruned matrices for comparison
# print("\nTrue B0:\n", (B0_true != 0).astype(int))
# if 'B0_est_pruned' in locals(): print("Estimated B0 (Pruned):\n", (B0_est_pruned != 0).astype(int))
# print("\nTrue B1:\n", (B1_true != 0).astype(int))
# if 'B1_est_pruned' in locals(): print("Estimated B1 (Pruned):\n", (B1_est_pruned != 0).astype(int))